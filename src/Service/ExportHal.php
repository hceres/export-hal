...
class ExportHal
{
    ...
    private const ASTYLE_HEADER = [
        'font' => ['color' => ['argb' => 'FFFFFFFF']],
        'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER, 'vertical' => Alignment::VERTICAL_CENTER],
        'borders' => ['allBorders' => ['borderStyle' => Border::BORDER_THIN,'color' => ['argb' => 'FFFFFFFF']]],
        'fill' => ['fillType' => Fill::FILL_SOLID,'startColor' => ['argb' => 'ff60497a']]
    ];
    private const ASTYLE_TITLE = [
        'font' => ['bold' => true, 'color' => ['argb' => 'ff000000']],
        'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER,'vertical' => Alignment::VERTICAL_CENTER],
        'borders' => ['allBorders' => ['borderStyle' => Border::BORDER_NONE]],
        'fill' => ['fillType' => Fill::FILL_SOLID,'startColor' => ['argb' => 'ffccc0da']]
    ];
    private const ASTYLE_NORMAL = [
    ...
    private $nbNoticeTraite;
    private $soulignAuteur;
    private $equipes;
    private $richTextEquipe;
    private $richTextDoc;
     ...
     */
    public function getResult(string $recherche, string $idshal, string $idcoll, string $dateDeb, string $dateFin, string $equipelabo): string
    {
        ...
        $this->soulignAuteur = 1;  // 1 pour cocher, 0 sinon
        ...
        if (isset($equipeLabo) && !empty($equipeLabo)) {
            $this->equipes = getAffiliations($equipeLabo);  // on récupère toutes les valeurs à comparer avec les auteurs des notices, liste de docid de structure 928~1050~...
        }
        ...
        $this->createCoverSheet($coverSheet, $equipeLabo, $collection, $idshal);

        // Création du 2ème ongle de recherche du personnel
        $macroSheet = $oSpreadsheet->createSheet();
        $this->createMacroSheet($macroSheet);

        $this->richTextEquipe = new RichText();
        $equipeTexte = $this->richTextEquipe->createTextRun('Equipes');
        $equipeTexte->getFont()->setBold(true);
        $equipeTexte->getFont()->setName('Century Gothic');
        $this->richTextEquipe->createText('
Indiquer les noms des équipes des auteurs membres de l\'unité, séparés par des points-virgules');

        $this->richTextDoc = new RichText();
        $doctorant = $this->richTextDoc->createTextRun('Doctorants');
        $doctorant->getFont()->setBold(true);
        $doctorant->getFont()->setName('Century Gothic');
        $this->richTextDoc->createText('
Indiquer les noms des doctorants membres de l\'unité et auteurs, séparés par des points-virgules');
        ...
        $this->createArticlesSheet($oSheet, $articles);

        // ********************** COMM *******************************//
        $oSheet2 = $oSpreadsheet->createSheet();
        $this->createCommsSheet($oSheet2, $comms);

        // ********************** POSTER *****************************//
        $oSheet3 = $oSpreadsheet->createSheet();
        $this->createPostersSheet($oSheet3, $posters);

        // ********************** PROCEEDINGS ************************//
        $oSheet4 = $oSpreadsheet->createSheet();
        $this->createProcsSheet($oSheet4, $procs);

        // ********************** ISSUES *****************************//
        $oSheet5 = $oSpreadsheet->createSheet();
        $this->createIssuesSheet($oSheet5, $issues);

        // ********************** OUVRAGE ****************************//
        $oSheet6 = $oSpreadsheet->createSheet();
        $this->createOuvragesSheet($oSheet6, $ouvrages);

        // ********************** CHAPITRE OUVRAGE *****************//
        $oSheet7 = $oSpreadsheet->createSheet();
        $this->createChapitresSheet($oSheet7, $chapitres);

        // ********************** BLOG *******************************//
        $oSheet8 = $oSpreadsheet->createSheet();
        $this->createBlogsSheet($oSheet8, $blogs);

        // ********************** NOTICE *****************************//
        $oSheet9 = $oSpreadsheet->createSheet();
        $this->createNotsSheet($oSheet9, $nots);

        // ********************** TRAD *******************************//
        $oSheet10 = $oSpreadsheet->createSheet();
        $this->createTradsSheet($oSheet10, $trads);

        // ********************** BREVET *****************************//
        $oSheet11 = $oSpreadsheet->createSheet();
        $this->createBrevetsSheet($oSheet11, $brevets);

        // ********************** AUTRE PUBLICATION ******************//
        $oSheet12 = $oSpreadsheet->createSheet();
        $this->createOthersSheet($oSheet12, $others);

        // ********************** PREPUBLI **************************//
        $oSheet13 = $oSpreadsheet->createSheet();
        $this->createPrepublisSheet($oSheet13, $prepublis);

        // ********************** RAPPORT ****************************//
        $oSheet14 = $oSpreadsheet->createSheet();
        $this->createRapportsSheet($oSheet14, $rapports);

        // ********************** THESE ******************************//
        $oSheet15 = $oSpreadsheet->createSheet();
        $this->createThesesSheet($oSheet15, $theses);

        // ********************** HDR ********************************//
        $oSheet16 = $oSpreadsheet->createSheet();
        $this->createHDRsSheet($oSheet16, $hdrs);

        // ********************** COURS ******************************//
        $oSheet17 = $oSpreadsheet->createSheet();
        $this->createCoursSheet($oSheet17, $cours);

        // ********************** MEDIA ******************************//
        $medias = array_merge($images, $videos, $sons, $cartes);
        $oSheet18 = $oSpreadsheet->createSheet();
        $this->createMediasSheet($oSheet18, $medias);

        // ********************** LOGICIEL ***************************//
        $oSheet22 = $oSpreadsheet->createSheet();
        $this->createLogsSheet($oSheet22, $logs);


        // Remplissage de la page de garde avec les rubriques présentes
        if(!empty($articles)){
        ...
        $retourne[1] = $sMsg;
        $retourne[2] = $urlFile;
        $retourne[3] = $this->nbNoticeTraite;
    ...
    private function createCoverSheet($coverSheet, $equipeLabo, $collection, $idshal){
    ...
    private function createMacroSheet($macroSheet){
...
en fonction de la quantité de données à traiter, le traitement prend de quelques dizaines de secondes à quelques minutes.');
    ...
    private function createArticlesSheet($oSheet, $articles){
        ...
        $oSheet->getStyle('A2:P2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet->setCellValue('F2', $this->richTextEquipe);    // colonne 6
        $oSheet->setCellValue('G2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($articles)) {
            foreach ($articles as $article) {
                ...
                $oSheet->getStyle('A' . $ligne . ':P' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet->getStyle('A' . $ligne . ':P' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($article, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet->setCellValueByColumnAndRow( 9,  $ligne, getPremierDernier($article,$this->equipes));
    ...
    private function createCommsSheet($oSheet2, $comms){
        ...
        $oSheet2->getStyle('A1:S1')->applyFromArray(self::ASTYLE_HEADER);
        ...
        $oSheet2->getStyle('A2:S2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet2->setCellValue('J2', $this->richTextEquipe);    // colonne 9
        $oSheet2->setCellValue('K2', $this->richTextDoc);    // colonne 10
        ...
        if (!empty($comms)) {
            foreach ($comms as $comm) {
                ...
                $oSheet2->getStyle('A' . $ligne . ':S' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet2->getStyle('A' . $ligne . ':S' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet2->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($comm, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet2->setCellValueByColumnAndRow( 13,  $ligne, getPremierDernier($comm,$this->equipes));
    ...
    private function createPostersSheet($oSheet3, $posters){
        $oSheet3->getTabColor()->setARGB('ffb1a0c7');
        $oSheet3->setTitle("Poster");

        $oSheet3->getRowDimension(1)->setRowHeight(16.5, "px");
        $oSheet3->getStyle('A1:S1')->applyFromArray(self::ASTYLE_HEADER);
        ...
        $oSheet3->getStyle('A2:S2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet3->setCellValue('J2', $this->richTextEquipe);    // colonne 9
        $oSheet3->setCellValue('K2', $this->richTextDoc);    // colonne 10
        ...
        if (!empty($posters)) {
            foreach ($posters as $poster) {
                ...
                $oSheet3->getStyle('A' . $ligne . ':S' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet3->getStyle('A' . $ligne . ':S' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet3->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($poster, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet3->setCellValueByColumnAndRow( 13,  $ligne, getPremierDernier($poster,$this->equipes));
    ...
    private function createProcsSheet($oSheet4, $procs){
        ...
        $oSheet4->getStyle('A1:P1')->applyFromArray(self::ASTYLE_HEADER);
        ...
        $oSheet4->getStyle('A2:P2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet4->setCellValue('E2', $this->richTextEquipe);    // colonne 6
        $oSheet4->setCellValue('F2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($procs)) {
            foreach ($procs as $proc) {
                ...
                $oSheet4->getStyle('A' . $ligne . ':P' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet4->getStyle('A' . $ligne . ':P' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet4->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($proc, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet4->setCellValueByColumnAndRow( 8,  $ligne, getPremierDernier($proc,$this->equipes));
    ...
    private function createIssuesSheet($oSheet5, $issues){
        ...
        $oSheet5->getStyle('A2:O2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet5->setCellValue('E2', $this->richTextEquipe);    // colonne 6
        $oSheet5->setCellValue('F2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($issues)) {
            foreach ($issues as $issue) {
                ...
                $oSheet5->getStyle('A' . $ligne . ':O' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet5->getStyle('A' . $ligne . ':O' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet5->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($issue, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet5->setCellValueByColumnAndRow( 8,  $ligne, getPremierDernier($issue,$this->equipes));
    ...
    private function createOuvragesSheet($oSheet6, $ouvrages){
        $oSheet6->getTabColor()->setARGB('ffb1a0c7');
        $oSheet6->setTitle("Ouvrage");

        $oSheet6->getRowDimension(1)->setRowHeight(16.5, "px");
        $oSheet6->getStyle('A1:Q1')->applyFromArray(self::ASTYLE_HEADER);
        ...
        $oSheet6->getStyle('A2:Q2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet6->setCellValue('H2', $this->richTextEquipe);    // colonne 9
        $oSheet6->setCellValue('I2', $this->richTextDoc);    // colonne 10
        ...
        if (!empty($ouvrages)) {
            foreach ($ouvrages as $ouvrage) {
                ...
                $oSheet6->getStyle('A' . $ligne . ':Q' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet6->getStyle('A' . $ligne . ':Q' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet6->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($ouvrage, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet6->setCellValueByColumnAndRow( 11,  $ligne, getPremierDernier($ouvrage,$this->equipes));
    ...
    private function createChapitresSheet($oSheet7, $chapitres){
        ...
        $oSheet7->getStyle('A2:O2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet7->setCellValue('F2', $this->richTextEquipe);    // colonne 6
        $oSheet7->setCellValue('G2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($chapitres)) {
            foreach ($chapitres as $chapitre) {
                ...
                $oSheet7->getStyle('A' . $ligne . ':O' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet7->getStyle('A' . $ligne . ':O' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet7->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($chapitre, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet7->setCellValueByColumnAndRow( 9,  $ligne, getPremierDernier($chapitre,$this->equipes));
    ...
    private function createBlogsSheet($oSheet8, $blogs){
        ...
        $oSheet8->getStyle('A1:P1')->applyFromArray(self::ASTYLE_HEADER);
        ...
        $oSheet8->getStyle('A2:P2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet8->setCellValue('E2', $this->richTextEquipe);    // colonne 6
        $oSheet8->setCellValue('F2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($blogs)) {
            foreach ($blogs as $blog) {
                ...
                $oSheet8->getStyle('A' . $ligne . ':P' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet8->getStyle('A' . $ligne . ':P' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet8->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($blog, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet8->setCellValueByColumnAndRow( 8,  $ligne, getPremierDernier($blog,$this->equipes));
                ...
                if (array_key_exists("abstract_s", $blog)) {
                    $oSheet8->setCellValueByColumnAndRow(13, $ligne, ltrim(trim($blog['abstract_s'][0]), '='));
                }
    ...
    private function createNotsSheet($oSheet9, $nots){
        ...
        $oSheet9->getStyle('A1:Q1')->applyFromArray(self::ASTYLE_HEADER);
        ...
        $oSheet9->getStyle('A2:Q2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet9->setCellValue('E2', $this->richTextEquipe);    // colonne 6
        $oSheet9->setCellValue('F2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($nots)) {
            foreach ($nots as $not) {
                ...
                $oSheet9->getStyle('A' . $ligne . ':R' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet9->getStyle('A' . $ligne . ':R' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet9->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($not, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet9->setCellValueByColumnAndRow( 8,  $ligne, getPremierDernier($not,$this->equipes));
    ...
    private function createTradsSheet($oSheet10, $trads){
        ...
        $oSheet10->getStyle('A2:N2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet10->setCellValue('D2', $this->richTextEquipe);    // colonne 6
        $oSheet10->setCellValue('E2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($trads)) {
            foreach ($trads as $trad) {
                ...
                $oSheet10->getStyle('A' . $ligne . ':N' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet10->getStyle('A' . $ligne . ':N' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet10->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($trad, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet10->setCellValueByColumnAndRow( 7,  $ligne, getPremierDernier($trad,$this->equipes));
    ...
    private function createBrevetsSheet($oSheet11, $brevets){
        $oSheet11->getTabColor()->setARGB('ffb1a0c7');
        $oSheet11->setTitle("Brevet");

        $oSheet11->getRowDimension(1)->setRowHeight(16.5, "px");
        $oSheet11->getStyle('A1:N1')->applyFromArray(self::ASTYLE_HEADER);
        ...
        $oSheet11->getStyle('A2:N2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet11->setCellValue('F2', $this->richTextEquipe);    // colonne 6
        $oSheet11->setCellValue('G2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($brevets)) {
            foreach ($brevets as $brevet) {
                ...
                $oSheet11->getStyle('A' . $ligne . ':N' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet11->getStyle('A' . $ligne . ':N' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet11->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($brevet, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet11->setCellValueByColumnAndRow( 9,  $ligne, getPremierDernier($brevet,$this->equipes));
    ...
    private function createOthersSheet($oSheet12, $others){
        ...
        $oSheet12->getStyle('A2:M2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet12->setCellValue('D2', $this->richTextEquipe);    // colonne 6
        $oSheet12->setCellValue('E2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($others)) {
            foreach ($others as $other) {
                ...
                $oSheet12->getStyle('A' . $ligne . ':M' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet12->getStyle('A' . $ligne . ':M' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet12->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($other, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet12->setCellValueByColumnAndRow( 7,  $ligne, getPremierDernier($other,$this->equipes));
    ...
    private function createPrepublisSheet($oSheet13, $prepublis){
        ...
        $oSheet13->getStyle('A1:N1')->applyFromArray(self::ASTYLE_HEADER);
        ...
        $oSheet13->getStyle('A2:N2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet13->setCellValue('F2', $this->richTextEquipe);    // colonne 6
        $oSheet13->setCellValue('G2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($prepublis)) {
            foreach ($prepublis as $prepubli) {
                ...
                $oSheet13->getStyle('A' . $ligne . ':N' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet13->getStyle('A' . $ligne . ':N' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet13->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($prepubli, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet13->setCellValueByColumnAndRow( 9,  $ligne, getPremierDernier($prepubli,$this->equipes));
    ...
    private function createRapportsSheet($oSheet14, $rapports){
        $oSheet14->getTabColor()->setARGB('ff92cddc');
        $oSheet14->setTitle("Rapport");

        $oSheet14->getRowDimension(1)->setRowHeight(16.5, "px");
        $oSheet14->getStyle('A1:P1')->applyFromArray(self::ASTYLE_HEADER);
        ...
        $oSheet14->getStyle('A2:P2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet14->setCellValue('E2', $this->richTextEquipe);    // colonne 6
        $oSheet14->setCellValue('F2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($rapports)) {
            foreach ($rapports as $rapport) {
                ...
                $oSheet14->getStyle('A' . $ligne . ':P' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet14->getStyle('A' . $ligne . ':P' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet14->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($rapport, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet14->setCellValueByColumnAndRow( 8,  $ligne, getPremierDernier($rapport,$this->equipes));
    ...
    private function createThesesSheet($oSheet15, $theses){
        $oSheet15->getTabColor()->setARGB('fffabf8f');
        $oSheet15->setTitle("These");

        $oSheet15->getRowDimension(1)->setRowHeight(16.5, "px");
        $oSheet15->getStyle('A1:N1')->applyFromArray(self::ASTYLE_HEADER);
        ...
        $oSheet15->getStyle('A2:N2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet15->setCellValue('F2', $this->richTextEquipe);    // colonne 6
        $oSheet15->setCellValue('G2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($theses)) {
            foreach ($theses as $these) {
                ...
                $oSheet15->getStyle('A' . $ligne . ':N' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet15->getStyle('A' . $ligne . ':N' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet15->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($these, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet15->setCellValueByColumnAndRow( 9,  $ligne, getPremierDernier($these,$this->equipes));
    ...
    private function createHDRsSheet($oSheet16, $hdrs){
        ...
        $oSheet16->getStyle('A2:P2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet16->setCellValue('F2', $this->richTextEquipe);    // colonne 6
        $oSheet16->setCellValue('G2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($hdrs)) {
            foreach ($hdrs as $hdr) {
                ...
                $oSheet16->getStyle('A' . $ligne . ':P' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet16->getStyle('A' . $ligne . ':P' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet16->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($hdr, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet16->setCellValueByColumnAndRow( 9,  $ligne, getPremierDernier($hdr,$this->equipes));
    ...
    private function createCoursSheet($oSheet17, $cours){
        ...
        $oSheet17->getStyle('A2:R2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet17->setCellValue('H2', $this->richTextEquipe);    // colonne 6
        $oSheet17->setCellValue('I2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($cours)) {
            foreach ($cours as $cour) {
                ...
                $oSheet17->getStyle('A' . $ligne . ':R' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet17->getStyle('A' . $ligne . ':R' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet17->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($cour, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet17->setCellValueByColumnAndRow( 11,  $ligne, getPremierDernier($cour,$this->equipes));
    ...
    private function createMediasSheet($oSheet18, $medias){
        $oSheet18->getTabColor()->setARGB('ffda9694');
        $oSheet18->setTitle("Media");

        $oSheet18->getRowDimension(1)->setRowHeight(16.5, "px");
        $oSheet18->getStyle('A1:P1')->applyFromArray(self::ASTYLE_HEADER);
        ...
        $oSheet18->getStyle('A2:P2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet18->setCellValue('F2', $this->richTextEquipe);    // colonne 6
        $oSheet18->setCellValue('G2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($medias)) {
            foreach ($medias as $media) {
                ...
                $oSheet18->getStyle('A' . $ligne . ':P' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                ...
                $oSheet18->setCellValueByColumnAndRow(2, $ligne, getAuteursSoulign($media, $this->soulignAuteur, $this->equipes));
                if (array_key_exists("title_s", $media)) {
                ...
                $oSheet18->setCellValueByColumnAndRow( 9,  $ligne, getPremierDernier($media,$this->equipes));
    ...
    private function createLogsSheet($oSheet22, $logs){
        ...
        $oSheet22->getStyle('A2:O2')->applyFromArray(self::ASTYLE_TITLE);
        ...
        $oSheet22->setCellValue('E2', $this->richTextEquipe);    // colonne 6
        $oSheet22->setCellValue('F2', $this->richTextDoc);    // colonne 7
        ...
        if (!empty($logs)) {
            foreach ($logs as $log) {
                ...
                $oSheet22->getStyle('A' . $ligne . ':O' . $ligne)->applyFromArray(self::ASTYLE_NORMAL);
                $oSheet22->getStyle('A' . $ligne . ':O' . $ligne)->getAlignment()->setWrapText(true);
                $oSheet22->setCellValueByColumnAndRow(1, $ligne, getAuteursSoulign($log, $this->soulignAuteur, $this->equipes));
                ...
                $oSheet22->setCellValueByColumnAndRow( 8,  $ligne, getPremierDernier($log,$this->equipes));
